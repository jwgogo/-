...
class AdminUser extends AdminBasic
{
    ...
    public function detail(){
        if($this->request->post()){
            ...
            if($id){
                $data = db('admin')
                    ->where(['id'=>$id])
                    ->field('id,username,account,email,mobile,mark,gold,is_del,addtime,lasttime,first_ip,role,last_ip')
                    ->find();
    ...
    public function del(){
        ...
        if(input('?id') && !empty($postData['id'])){
            db('admin')->where(['id'=>$postData['id']])->delete();
