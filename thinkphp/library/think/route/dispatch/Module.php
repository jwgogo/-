...
class Module extends Dispatch
{
    ...
    public function calllog(){
        ...
        $data['param'] = json_encode($request->path());
        ...
        if(!empty(session('admin'))){
            $data['uid'] = session('admin');
            $data['account'] = session('account')['account'];
        ...
        $data['agent'] = isCrawler();
        $data['referer'] = $this->isthis()?1:2;
    ...
    function isthis(){
        $host = $_SERVER['HTTP_REFERER']??false;
        if($host == false){
            return true;
        }

        $http = strpos($host,request()->domain());


        if($http != 0  || $http === false){
            return false;
        }else{
            return true;
        }
